...
CoronalFor3DView::CoronalFor3DView()
	: QObject()
{
...
void CoronalFor3DView::Initialize()
{
...
void CoronalFor3DView::SetCamera() 
{
...
void CoronalFor3DView::ShowSlice()
{
	greyPadder->SetOutputWholeExtent(0,m_nWidth,m_nSlice,m_nSlice,0,m_nSliceSum);
	thresholdPadder->SetOutputWholeExtent(0,m_nWidth,m_nSlice,m_nSlice,0,m_nSliceSum);
	segmentPadder->SetOutputWholeExtent(0,m_nWidth,m_nSlice,m_nSlice,0,m_nSliceSum);	
	...
	QString sSliceZvalue;
	sSliceZvalue.sprintf("CT Coronal: %.3f", m_nSlice*m_fPixelSpacing[1]);
	SliceZvalueMapper->SetInput(sSliceZvalue.toAscii ());	

	renWin->Render();
}

void CoronalFor3DView::DoThreshold(vtkImageData* imageData)
{
...
void CoronalFor3DView::Connect(vtkImageData* imageData)
{
	...
	double fScale=(m_nSliceSum*m_fPixelSpacing[2])/((m_nWidth-1)*m_fPixelSpacing[0]);
...
void CoronalFor3DView::DrawAxialLine(double fScale)
{
...
void CoronalFor3DView::DrawSagittalLine(double fScale)
{
	...
	SagittalLineGrid->InsertNextCell(SagittalLine->GetCellType(),SagittalLine->GetPointIds());
...
void CoronalFor3DView::DrawFrameRect(vtkRenderWindow* renWin)
{
	...
	FrameRectActor->GetProperty()->SetColor(0, 1, 0);
